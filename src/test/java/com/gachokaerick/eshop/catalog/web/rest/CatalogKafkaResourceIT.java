...
import com.gachokaerick.eshop.catalog.config.KafkaProperties;
...
class CatalogKafkaResourceIT {
    ...
    void setup() {
        ...
        CatalogKafkaResource kafkaResource = new CatalogKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/catalog-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/catalog-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
